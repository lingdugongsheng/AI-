...
class SearchResult(BaseModel):
    """搜索结果数据模型 - 表示单条搜索结果的信息"""
    title: str  # 搜索结果的标题
    source: str  # 信息来源（如：科技新闻网、计算机学报等）
    url: str  # 原文链接地址
    snippet: str  # 摘要片段（简短的内容预览）
    relevance_score: float = Field(  # 相关性评分（0.0-1.0）
        ge=0.0,  # 最小值约束：大于等于0.0
        le=1.0  # 最大值约束：小于等于1.0
    )
    publish_date: Optional[str] = None  # 发布日期（可选字段）


class ResearchFinding(BaseModel):
    """研究发现数据模型 - 表示从资料分析中提取的关键发现"""
    topic: str  # 研究主题
    key_points: list[str]  # 核心要点列表（从资料中提取的主要观点）
    evidence: list[str]  # 支持证据列表（支持观点的具体证据）
    confidence: float = Field(  # 置信度评分（0.0-1.0）
        ge=0.0,  # 最小值约束
        le=1.0  # 最大值约束
    )
    sources: list[str]  # 信息来源列表（引用的文献标题或ID）


class ResearchOutline(BaseModel):
    """研究大纲数据模型 - 表示研究的整体框架和结构"""
    title: str  # 研究报告的标题
    abstract: str  # 摘要（简要概述研究内容，通常100-200字）
    sections: list[str]  # 章节列表（报告的主要章节结构）
    key_questions: list[str]  # 核心研究问题列表（需要回答的关键问题）
    methodology: str  # 研究方法论（采用的研究方法和步骤）
...
class Citation(BaseModel):
    ...
    title: str  # 文献标题
    source: str  # 出版来源（期刊、会议、网站等）
    year: int  # 出版年份
    url: Optional[str] = None  # 在线资源链接（可选字段）


class ResearchReport(BaseModel):
    """研究报告数据模型 - 表示完整的学术研究报告"""
    title: str  # 报告标题
    executive_summary: str  # 执行摘要（面向决策者的简短总结）
    introduction: str  # 引言（研究背景、目的和意义）
    methodology: str  # 研究方法（详细说明研究过程和方法）
    findings: list[str]  # 研究发现列表（主要发现和结果）
    analysis: str  # 分析讨论（对发现的深入分析和解释）
    conclusions: list[str]  # 结论列表（研究的主要结论）
    recommendations: list[str]  # 建议列表（基于研究的实践建议）
    citations: list[Citation]  # 参考文献列表（所有引用的文献）
    generated_at: str  # 生成时间（报告生成的时间戳）


# ==================== 状态定义（LangGraph状态管理） ====================
# ResearchState用于在LangGraph工作流中管理研究助手的状态
# 使用TypedDict提供类型提示，增强代码可读性和类型安全

class ResearchState(TypedDict):
    """研究助手状态 - 定义整个研究工作流中需要维护的状态信息"""

    # ========== 消息历史 ==========
    messages: Annotated[list, add_messages]  # 对话消息历史（用户和AI的交互记录）
    # 使用add_messages注解支持消息累加

    # ========== 研究主题信息 ==========
    research_topic: str  # 当前研究的主题
    research_questions: list[str]  # 需要回答的研究问题列表

    # ========== 收集的数据 ==========
    search_results: list[dict]  # 搜索结果列表（包含学术和网络搜索结果）
    analyzed_sources: list[dict]  # 已分析的资料列表（经过处理和评估的来源）

    # ========== 研究进展 ==========
    outline: dict  # 研究大纲（ResearchOutline的字典形式）
    findings: list[dict]  # 研究发现列表（ResearchFinding的字典形式）

    # ========== 报告生成 ==========
    draft_sections: dict  # 报告草稿的各章节内容（键值对形式）
    final_report: str  # 完整的最终研究报告（格式化后的文本）
    citations: list[dict]  # 参考文献列表（Citation的字典形式）

    # ========== 状态追踪 ==========
    current_phase: str  # 当前研究阶段（如：planning, analysis等）
    iteration_count: int  # 迭代次数（用于控制质量检查的循环次数）
    quality_score: float  # 报告质量评分（0.0-10.0，用于决定是否继续迭代）
